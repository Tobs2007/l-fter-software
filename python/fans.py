...
layout = [  [sg.Text('Choose theme')],
            [sg.Button('System info'), sg.Button('test_pattern'), sg.Button('exit')],
             [sg.Button('anim1'), sg.Button('anim2'),sg.Button('hue'),sg.Button('screen'),sg.Button('close')],
             ...
             #[sg.Text("Huemode:"),sg.InputText(default_text='0',enable_events=True,key='huemode',size=(5,1))],
             [sg.Text("enable Hue"),sg.Checkbox(enable_events=True,key="hueenableui",default=False,text=""),sg.Button('normalize')],
...
def hueshift(speed):#not working
    ...
    for hue in range(int(255/speed)):
        ...
        color = list(colorRaw)
        globals.gR=color[0]*255
        globals.gG=color[1]*255
        globals.gB=color[2]*255
...
def screenColor():
    with mss() as sct:
        sct.shot()
    myimg = cv2.imread('monitor-1.png')
    avg_color_per_row = numpy.average(myimg, axis=0)
    avg_color = numpy.average(avg_color_per_row, axis=0)
    print(avg_color)
    for x in range (8):
        setFansNoShow(x,avg_color[2]*1.2,avg_color[1],avg_color[0])
        setled(3,x,avg_color[2]*1.2,avg_color[1],avg_color[0])
    show()
def main():#main loop controlling what scheme is displayed
    ...
    while True:
        ...
        if state==0:#state is written in gui()
            ...
            fps = 10
            system_info(globals.gR,globals.gG,globals.gB,0.01)
        elif state==2:
            fps = 1
            test_patern()
        elif state==3:
            fps = 20
            anim1(globals.gR,globals.gG,globals.gB)
        elif state==4:
            fps = 20
            meter(0.0005)
        elif state==5:
...
def gui():
    ...
    while True:
        ...
        # if event == 'huemode' and not values['huemode']:
        #     values['huemode']="0"
        # if event == 'huemode' and values['huemode'] and values['huemode'][-1] not in ('0123456789'):
        #     window['huemode'].update(values['huemode'][:-1])
        #     values['huemode']=values['huemode'][:-1]
        # if event == 'huemode' and not values['huemode']:
        #     values['huemode']="0"
        # if event == 'huemode' and int(values['huemode'])>255:
        #     window['huemode'].update(values['huemode'][:-1])
        #     values['huemode']=values['huemode'][:-1]
        # if event == 'huemode' and int(values['huemode'])<0:
        #     window['huemode'].update(values['huemode'][:-1])
        #     values['huemode']=values['huemode'][:-1]
        # globals.huemode=int(values['huemode'])
        globals.hueenable=bool(values["hueenableui"])
        globals.gR=int(values['R'])#setting global rgb values
        globals.gG=int(values['G'])
        globals.gB=int(values['B'])
